function [Vx,Vy,reliab]=myoptFlowLk( I1, I2, winN, ...
...
%I1 = imgaussfilt(I1,sigma,padding=0);
%I2 = imgaussfilt(I2,sigma,padding=0);
...
if( isempty(winSig) )
  ...
  winN = ceil(winSig);
  Axx=imgaussfilt(Gxx,winSig,padding=0);
  Axy=imgaussfilt(Gxy,winSig,padding=0);
  Ayy=imgaussfilt(Gyy,winSig,padding=0);
  %Axx=imgaussfilt(Gxx,winSig);
  %Axy=imgaussfilt(Gxy,winSig);
  %Ayy=imgaussfilt(Gyy,winSig);
end;
% 
...
if( isempty(winSig) )
  ...
  %ATbx=imgaussfilt(IxIt,winSig,padding=0);
  %ATby=imgaussfilt(IyIt,winSig,padding=0);
  ATbx=imgaussfilt(IxIt,winSig);
  ATby=imgaussfilt(IyIt,winSig);
end;
